...
class search_control extends phpok_control
{
	...
	public function index_f()
	{
		...
		$myshownums = $this->get('myshownums');
		$cateid = $this->get('cateid');
		if($keywords){
			$keywords = str_replace(array("　","，",",","｜","|","、","/","\\","／","＼","+","＋")," ",$keywords);
			$keywords = trim($keywords);
			if($keywords){
				$this->load_search($keywords,$myshownums,$cateid);
			}
		}
		$this->view("search_index");
	}

	private function load_search($keywords,$myshownums,$cateid)
	{
		...
		if($cateid){
			$condition = "l.project_id IN(".implode(",",$pids).") AND l.module_id IN(".implode(",",$mids).") AND l.cate_id IN(".$cateid.") ";
		}else{
